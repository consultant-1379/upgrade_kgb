...
public class LiveUpgTarFileDownloadTestSteps extends TorTestCaseHelper {

    final static Logger logger = LoggerFactory.getLogger(LiveUpgTarFileDownloadTestSteps.class);
    ...
    {    
        ...
        curlProxyUser=(String) configuration.getProperty("testware.curlProxyUser");
    	 ...
    	 depoPath= upgTarFileOperator.getDepoPath() + "/slu";
    	omMediaCachePath=(String) configuration.getProperty(upgTarFileOperator.OM_CACHE);
		documentNumber="19090-"+ upgTarFileOperator.getLiveUpgradeCxpNumber(omMediaCachePath);
		assertNotNull("Orcha proxy user is missing ",curlProxyUser);
		 ...
		 assertNotNull("Media cache path is empty  ", omMediaCachePath);
    	...
    	assertEquals("Downloading of Tar file curl commands is not executed properly",upgTarFileOperator.downloadTarToGateway(downloadLocation,documentNumber,tarFileName),0);
   ...
   /* Verifies the downloaded file integrity*/
    ...
    {     
        ...
        assertEquals("  tar file is not copied to  the depo path ",upgTarFileOperator.verifyPathAvailabilityOnMws(depoPath+"/"+tarFileName),0);
